...
def test_tcp_udp_turn(server_address, username, password, realm, server_hostname, use_tls, target_ip="8.8.8.8", target_port=53, use_short_term_credential=False, sni_hostname=None, send_sni=True):
    ...
    try:
        ...
        control_sock, _, _, _, actual_server_address, *extra = result
        if len(extra) > 0:
            mi_algorithm = extra[0]
            print(f"[+] 服务器消息完整性算法: {mi_algorithm}")
        if is_short_term:
            print("[+] Using short-term credential for allocation")
        print(f"✅ TCP+UDP TURN分配成功 (实际服务器: {actual_server_address})")
        print("[i] 根据测试要求，到 Allocate 成功即视为能力正常。")
        control_sock.close()
        return True
